...
// CLI код
// NODE_ENV=development node main.js -> true
// NODE_ENV=prod node main.js -> false

// або

// node --env-file=.env.example main.js
...
function encodeToBase64(...args) {
  ...
  const encoded = Buffer.from(result, "utf-8").toString("base64");
  // console.log(`Encoded Base64: ${encoded}`);

  return encoded;
}

// encodeToBase64("Hello World! 1", "Hello World! 2"); // SGVsbG8gV29ybGQhIDEgOiBIZWxsbyBXb3JsZCEgMg==
...
// encodeToHex("Hello World! 1", "Hello World! 2"); // 48656c6c6f20576f726c64212031203a2048656c6c6f20576f726c64212032
...
// decodeFromBase64("SGVsbG8gV29ybGQhIDEgOiBIZWxsbyBXb3JsZCEgMg=="); // Hello World! 1 : Hello World! 2
...
// decodeFromHex("48656c6c6f20576f726c64212031203a2048656c6c6f20576f726c64212032"); // Hello World! 1 : Hello World! 2
...
function safeDecodeFromBase64(base64String) {
  ...
  const symbolCheck = base64RegExp.test(base64String);
  const lengthCheck = base64String.length % 4 === 0;

  const errors = [];

  if (!symbolCheck) {
    errors.push(
      "The symbols are incorrect for base64format. Allowed symbols are: a-z, A-Z, 0-9, +, /, = (in the end as a padding)",
    );
  }

  if (!lengthCheck) {
    errors.push("The length of the string is incorrect. Base64 output always appears in multiples of 4 characters.");
  }
...
function safeDecodeFromHex(hexString) {
  const hexRegExp = /^[a-fA-F0-9]+$/;

  const symbolCheck = hexRegExp.test(hexString);
  const lengthCheck = hexString.length % 2 === 0;

  const errors = [];

  if (!symbolCheck) {
    errors.push("The symbols are incorrect for Hex format. Allowed symbols are: a-f, A-F, 0-9");
  }

  if (!lengthCheck) {
    errors.push("The length of the string is incorrect. Hex output always appears in pairs of 2 characters.");
  }
  ...
  const encoded = Buffer.from(hexString).toString("hex");
  // console.log(`Encoded hex: ${encoded}`);
  return encoded;
}
