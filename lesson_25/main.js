...
function setUserInfoCookie(key, value) {
  const timerSec = 10;
  const cookie = encodeURIComponent(`${key}=${value}`);
  const expiration = `max-age=${timerSec}`;
  document.cookie = `userInfo=${cookie}; ${expiration}; path=/`;
  console.log(`The cookie ${key}=${value} has been successfully saved!`);
}
...
function getUserInfo(key) {
  ...
  console.log(`Retreived ${key}: ${value}`);
  return value;
}
