...
function LogMethodCalls(
  ...
  propertyDescriptor.value = function (...args: any[]) {
    ...
    console.log(`Calling "${propertyName}" with arguments: ${argsStr}`);
...
namespace UserProfile {
  ...
  // Не зовсім певен, як саме краще зробити рандомний ID, який буде стабільно і унікально генеруватись для великої кількості користувачів (без купи перегенерацій), тому зробив перевірений спосіб з інкрементом лічильника.
  function generateId(): string {
    id = `up_${count}`;
    count++;
    idArray.push(id);
    // console.log(`id: ${id}; count: ${count}; idArray: ${idArray}`);
    return id;
  }
  ...
  export function createProfile(
    ...
    if (typeof id === "undefined") {
      ...
      throw new Error(
        `ID ${id} already existst. Choose another one or do not specify any ID for automatic assign`,
      );
    }
