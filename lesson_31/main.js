...
// Функція відображення асинхронного результату (щоб менше було повторюваного коду на 5 рядків)
async function asyncResultDisplay(func, logName, ...args) {
  console.log("-----------------------");
  console.log(`${logName} method`);
...
async function getData(segment) {
  try {
    ...
    fetchResult = JSON.stringify(fetchResult);
...
const bodyPut = {
  ...
  id: 1,
  title: "Village intro",
  body: "Where is everyone going? Bingo?",
};
...
async function main() {
  await asyncResultDisplay(getData, "GET", segment);
  await asyncResultDisplay(postData, "POST", "/posts", bodyPost);
  await asyncResultDisplay(putData, "PUT", 1, bodyPut);
  await asyncResultDisplay(patchData, "PATCH", 1, bodyPatch);
  await asyncResultDisplay(deleteData, "DELETE", 10);
}
